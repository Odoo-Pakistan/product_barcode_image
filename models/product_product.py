...
class ProductProduct(models.Model):
    ...
    def _get_barcode_image(self):
        if self.ean13:
            ...
            self.barcode_image_jpg = base64.encodestring(bc_data_jpg)
            ...
            self.barcode_image_gif = base64.encodestring(bc_data_gif)

            barcode_bmp = createBarcodeDrawing('EAN13', value=str(self.ean13), format='bmp', width=float(width), height=float(height))
            bc_data_bmp = barcode_bmp.asString('bmp')
            self.barcode_image_bmp = base64.encodestring(bc_data_bmp)
    ...
    barcode_image_bmp = fields.Binary(compute=_get_barcode_image, string='Barcode Image (BMP)', store=False)
